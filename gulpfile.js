...
gulp.task("css_blocks", function () {
  gulp
    .src(config.paths.css.blocks.src)
    .pipe(sourcemaps.init())
    .pipe(plumber())
    .pipe(sass())
    .pipe(autoprefixer())
...
gulp.task(
  "watch",
  ["css_theme", "css_blocks", "css_vendor", "js_theme", "js_vendor", "fonts", "images","browsersync"],
  function () {
    gulp.watch(config.paths.watch.css.theme, ["css_theme"]);
    gulp.watch(config.paths.watch.css.blocks, ["css_blocks"]);
